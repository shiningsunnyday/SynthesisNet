...
class MCTSNode:
    ...
    def __init__(self, state, n_actions, TreeEnv, action=None, parent=None, policy=None):
        """
        ...
        """
        if max(sum(state[4096:4096+91]), sum(state[4096+91:4096+2*91])) > 1:
            breakpoint()
        if policy is None:
            breakpoint()
        self.policy = policy
        ...
        self.mask = self.policy.action_mask(state_tensor).flatten() # mask of valid actions
        self.children = {}
    ...
    def select_leaf(self):
        ...
        while True:
            ...
            # if sum(current.state[4096:]) == 2:
            #     breakpoint()
            # if sum(current.state[4096:]) == 1:
            #     breakpoint()                
            action_score = current.child_action_score
            action_score[~current.mask] = float("-inf")     
...
class MCTS:
    ...
    def __init__(self, agent_netw, TreeEnv, seconds_per_move=None,
                 simulations_per_move=800, num_parallel=2):
        """
    ...
    def pick_action(self):
        ...
        """
        if ((~self.root.mask)*self.root.child_N).sum().item():
            breakpoint()
